...
possible_n <- 1:max_value
for(n_value in possible_n) {
  possible_s <- {n_value + 1}:max_value
  for(s_value in possible_s) {
    possible_i <- {s_value + 1}:max_value
    for(i_value in possible_i) {
      possible_o <- {i_value + 1}:max_value
      for(o_value in possible_o) {
	    possible_d <- {o_value + 1}:max_value
		for(d_value in possible_d) {
		  m_value <- n_value + o_value
		  if(!{m_value > d_value}) next
		  possible_h <- {m_value + 1}:max_value
		  for(h_value in possible_h) {
			...
			possible_p <- {g_value + 1}:max_value
			possible_p <- possible_p[possible_p < o_value + q_value]
			possible_p <- possible_p[possible_p < g_value + i_value]
			...
			for(p_value in possible_p) {
			  ...
			  #if(!{b_value + f_value + m_value + q_value + t_value == a_value + b_value + c_value}) next
