...
reduce_possibilities <- function(values, the_letters = all_letters, times = 5) {

  values <- lapply(the_letters, reduced_possible_values, values)
 
  minimum_values <- lapply(values, min)
  maximum_values <- lapply(values, max)
  new_minimum_values <- minimum_values
  new_maximum_values <- maximum_values

    new_minimum_values <- within(new_minimum_values, {a <- max(a, b + f, b + h, c + e)})
    new_minimum_values <- within(new_minimum_values, {b <- max(b, f + g, c + d, m + o)})
    new_minimum_values <- within(new_minimum_values, {c <- max(c, d + e)})
    new_minimum_values <- within(new_minimum_values, {e <- max(e, d + h)})
    new_minimum_values <- within(new_minimum_values, {f <- max(f, m + n)})
    new_minimum_values <- within(new_minimum_values, {g <- max(g, f + n, h + i)})
    new_minimum_values <- within(new_minimum_values, {j <- max(j, i + p)})
    new_minimum_values <- within(new_minimum_values, {k <- max(k, s + l, m + q, p + n)})
    new_minimum_values <- within(new_minimum_values, {l <- max(l, m + q, n + o + q)})
    new_minimum_values <- within(new_minimum_values, {m <- max(m, n + o)})
    new_minimum_values <- within(new_minimum_values, {q <- max(q, m + o)})
    new_minimum_values <- within(new_minimum_values, {r <- max(r, p + j, j + i)})
    new_minimum_values <- within(new_minimum_values, {u <- max(u, s + k)})
    new_minimum_values <- as.list(pmax(unlist(new_minimum_values), unlist(minimum_values)))

    new_maximum_values <- within(new_maximum_values, r <- new_maximum_values[["p"]] + new_maximum_values[["j"]])
    new_maximum_values <- within(new_maximum_values, b <- new_maximum_values[["a"]] - new_minimum_values[["f"]])
    new_maximum_values <- within(new_maximum_values, c <- new_maximum_values[["b"]] - new_minimum_values[["d"]])
    new_maximum_values <- within(new_maximum_values, d <- new_maximum_values[["b"]] - new_minimum_values[["c"]])
    new_maximum_values <- within(new_maximum_values, e <- new_maximum_values[["c"]] - new_minimum_values[["d"]])
    new_maximum_values <- within(new_maximum_values, f <- new_maximum_values[["a"]] - new_minimum_values[["b"]])
    new_maximum_values <- within(new_maximum_values, h <- new_maximum_values[["g"]] - new_minimum_values[["i"]])
    new_maximum_values <- within(new_maximum_values, i <- new_maximum_values[["g"]] - new_minimum_values[["h"]])
    new_maximum_values <- within(new_maximum_values, j <- new_maximum_values[["r"]] - new_minimum_values[["p"]])
    new_maximum_values <- within(new_maximum_values, l <- new_maximum_values[["k"]] - new_minimum_values[["s"]])
    new_maximum_values <- within(new_maximum_values, m <- min(
      new_maximum_values[["q"]] - new_minimum_values[["o"]], 
	  new_maximum_values[["l"]] - new_minimum_values[["q"]]))
    new_maximum_values <- within(new_maximum_values, n <- new_maximum_values[["m"]] - new_minimum_values[["o"]])
    new_maximum_values <- within(new_maximum_values, p <- new_maximum_values[["r"]] - new_minimum_values[["j"]])
    new_maximum_values <- within(new_maximum_values, s <- new_maximum_values[["t"]] - new_minimum_values[["u"]])
    new_maximum_values <- within(new_maximum_values, u <- new_maximum_values[["t"]] - new_minimum_values[["s"]])
	new_maximum_values <- as.list(pmin(unlist(new_maximum_values), unlist(maximum_values)))
	
   
	new_maximum_values <- as.list(pmin(unlist(new_maximum_values), unlist(maximum_values)))
	
	values <- lapply(the_letters, FUN = function(the_letter) values[[the_letter]][values[[the_letter]] >= new_minimum_values[[the_letter]]])
    values <- lapply(the_letters, FUN = function(the_letter) values[[the_letter]][values[[the_letter]] <= new_maximum_values[[the_letter]]])

	used <- unlist(values[lapply(values, length) == 1], use.names = TRUE)
    values <- lapply(values, FUN = function(letter_values) letter_values[!{letter_values %in% used}])
	values[names(used)] <- used
  return(values)
}


for(i in 1:10) all_possible_values <- reduce_possibilities(all_possible_values, times = 20)


all_possible_values[["a"]]

new_possible_values        <- all_possible_values
new_possible_values[["a"]] <- new_possible_values[["a"]][1]
for(i in 1:10) new_possible_values <- reduce_possibilities(new_possible_values, times = 20)
all(lapply(new_possible_values, length) > 0)

possible_value_for_letter <- function(letter, value, possible_values) {
  if(value %in% possible_values[[letter]]) {
    possible_values[[letter]] <- value
    for(i in 1:10) possible_values <- reduce_possibilities(possible_values, times = 20)
    return(all(lapply(possible_values, length) > 0)) 
  } else {
    return(FALSE)
  }
}

possible_value_for_letter("a", 49, all_possible_values)
possible_value_for_letter("s", 21, all_possible_values)

new_possible_values        <- all_possible_values
new_possible_values[["a"]] <- new_possible_values[["a"]][sapply(new_possible_values[["a"]], FUN = function(value) possible_value_for_letter("a", value, new_possible_values))]
new_possible_values[["b"]] <- new_possible_values[["b"]][sapply(new_possible_values[["b"]], FUN = function(value) possible_value_for_letter("b", value, new_possible_values))]

for(i in 1:5) new_possible_values <- lapply(all_letters, FUN = function(letter) new_possible_values[[letter]][sapply(new_possible_values[[letter]], FUN = function(value) possible_value_for_letter(letter, value, new_possible_values))])
...
working_combination <- function(test_all_possible_values) {
  ...
  #message(number_of_calls)
  next_possible_values <- reduce_possibilities(test_all_possible_values)
  #non_length_1 <- unlist(lapply(test_all_possible_values, FUN = function(x) length(x) > 1))
  possibility_lengths <- unlist(lapply(next_possible_values, length))
  if(any(possibility_lengths == 0)) return(FALSE)
  non_length_1 <- possibility_lengths > 1
  #message("\n\n", non_length_1, "\n")
  if(sum(non_length_1) > 1) {
    of_max_length <- possibility_lengths == max(possibility_lengths)
    first_non_length_1 <- names(of_max_length[of_max_length][1])
    ...
    if(sum(non_length_1) == 1) {
      ...
      print(unlist(next_possible_values))
...
tst_all_possible_values <- new_possible_values
#tst_all_possible_values[["a"]] <- 50
#tst_all_possible_values[["b"]] <- 35
tst_all_possible_values <- reduce_possibilities(tst_all_possible_values)
tst_all_possible_values <- tst_all_possible_values[order(unlist(lapply(tst_all_possible_values, length)))]

# used <- unlist(tst_all_possible_values[lapply(tst_all_possible_values, length) == 1], use.names = FALSE)
# tst_all_possible_values_a <- lapply(tst_all_possible_values, FUN = function(letter_values) letter_values[!{letter_values %in% used}])
...
all_possible_values[["b"]]
all_possible_values[["f"]]
# since a = b + f, a must be at least(min(a) + min(f))
...
# start with all_possible_values
possible_values <- all_possible_values

max_values <- lapply(all_possible_values, max)
max_values <- lapply(all_possible_values, FUN = function(x) 80)

while(!solved) {
  # set n:
  possible_values[["n"]] <- possible_values[["n"]][1]
  n <- 
}
...
possible_n <- 2*{1:{max_value / 2}}
for(n_value in possible_n) {
  possible_s <- {n_value + 1}:max_values[["s"]]
  for(s_value in possible_s) {
    possible_i <- {s_value + 1}:max_values[["i"]]
    for(i_value in possible_i) {
      possible_o <- {i_value + 1}:max_values[["o"]]
      for(o_value in possible_o) {
	    possible_d <- {o_value + 1}:max_values[["d"]]
		for(d_value in possible_d) {
		  m_value <- n_value + o_value
		  if(!{m_value > d_value}) next
		  h_value <- 4 * d_value + m_value - 5 * i_value - n_value
		  if(!{h_value > m_value}) next
		  f_value <- h_value + i_value - n_value
		  if(!{f_value > h_value}) next
		  if(!{f_value > m_value + n_value}) next
	      q_value <- m_value + o_value
		  if(!{q_value > f_value}) next
	      g_value <- h_value + i_value
		  if(!{g_value > q_value}) next
          p_value <- n_value + g_value + i_value - o_value
		  if(!{p_value > g_value}) next
		  if(!{p_value < o_value + q_value}) next
		  if(!{p_value < g_value + i_value}) next
	      #browser()
	      e_value <- d_value + h_value
		  if(!{e_value > p_value}) next
	      j_value <- i_value + p_value
		  if(!{j_value > e_value}) next
		  if(!{j_value < h_value + e_value}) next
	      l_value <- m_value + q_value
		  if(!{l_value > j_value}) next
	      c_value <- d_value + e_value
		  if(!{c_value > l_value}) next
		  if(!{c_value > i_value + j_value}) next
	      k_value <- s_value + l_value
		  if(!{k_value > c_value}) next
	      u_value <- k_value + s_value
		  if(!{u_value > k_value}) next
		  if(!{u_value < l_value + q_value}) next
	      b_value <- c_value + d_value
		  if(!{b_value > u_value}) next
		  if(!{b_value > f_value + g_value}) next
		  if(!{b_value < f_value + g_value + h_value}) next
		  if(b_value + c_value != k_value + u_value) next
	      t_value <- s_value + u_value
		  if(!{t_value > b_value}) next
	      r_value <- p_value + j_value
		  if(!{r_value > t_value}) next
	      a_value <- b_value + f_value
 		  if(!{a_value > r_value}) next
		  if(!{f_value + m_value + q_value + t_value == a_value + c_value}) next
		  if(!{a_value + b_value + c_value == u_value + t_value + r_value}) next
		  			  
		  possible_solutions[[i]] <- c(
		    a = a_value, 
		  b = b_value, 
		  c = c_value, 
		  d = d_value, 
